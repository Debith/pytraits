...
# Omit unicode_literals here as we are dealing with code objects, who don't want
# handle any unicode stuff.
...
class Compiler:
    ...
    def _transfer_names(self, trait, clazz):
        ...
        for name in trait["co_names"]:
            if "__" not in name or is_sysname(name):
                ...
                varname = "_%s%s" % (clazz.__name__, name[name.index('__'):])
    ...
    def recompile(self, function, target, name):
        """
